...
def get_se_pack(card, sheet):
    ...
    pack_map = {
        ...
        'fhvp': 'TheFeastOfHemlockValePlayerExpansion',
        ...
        'enc': 'ParallelInvestigators',
        'tdcc': 'TheDrownedCity',
        'film_fatale': 'FilmFatale',
        'hoth': 'Promos',
        'tdor': 'Promos',
        'iotv': 'Promos',
        'tdg': 'Promos',
        ...
        if os.path.isfile(taboo_en_file):
            ...
            if active:
                for taboo_entry in active.get('cards', []):
                    ...
                    # Keep original translated text; only apply non-text changes (xp, cost, etc.).
                    # Cards with a full PT translation in taboo.json are handled above and skip this block.
                    text_fields = {'text', 'real_text', 'back_text', 'real_back_text'}
                    for key, value in taboo_entry.items():
                        if key == 'code' or key in text_fields:
                            continue
                        t_card[key] = value
...
def is_translatable(metadata):
    ...
    return metadata.get('id', None) and ('-m' not in metadata['id'])
